...
function generateFilterSlider(id, { min, max, values, step }) {
  ...
  var rangeH_result = $(id + ' .slider-result');
  rangeH.slider({
    range: true,
    min,
    max,
    ...
    slide: function( event, ui ) {
      rangeH_result.find('.min').text(ui.values[0]);
      rangeH_result.find('.max').text(ui.values[1]);
  ...
  rangeH_result.find('.min').text(rangeH.slider("values", 0));
  rangeH_result.find('.max').text(rangeH.slider("values", 1));
